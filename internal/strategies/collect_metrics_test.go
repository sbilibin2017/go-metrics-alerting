...
func TestGaugeMetricsCollector(t *testing.T) {
	collector := NewGaugeMetricsCollector()
	...
	for _, metric := range metrics {
		...
		assert.Equal(t, "gauge", metric.MType, "Metric type should be Gauge")
...
func TestCounterMetricsCollector(t *testing.T) {
	collector := NewCounterMetricsCollector()
	...
	assert.Equal(t, "counter", firstMetrics[0].MType, "Metric type should be Counter")
